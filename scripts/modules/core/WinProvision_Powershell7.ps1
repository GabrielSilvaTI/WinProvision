...
<#
.SYNOPSIS
    Verifica se o PowerShell 7+ está instalado; se não estiver, instala a versão
    mais recente diretamente do GitHub (release "latest").

.DESCRIPTION
    Script 100% autônomo, pensado para execução via ferramentas de automação
    (RMM, Intune, GPO Startup Script, etc.). Retorna códigos de saída
    consistentes para permitir checagem de sucesso/falha pela ferramenta chamadora.
...
.NOTES
    ...
    Códigos de saída:
        ...
        2 = Falha ao consultar a API do GitHub
        3 = Asset (MSI) não encontrado no release
        4 = Falha no download do MSI
        5 = Falha na instalação (msiexec)
        6 = Falha na verificação pós-instalação
#>

# ── Alta performance: desativa a barra de progresso (acelera muito o download) ──
...
# ── 1. Checagem ultra rápida (presença + versão, sem spawnar processo) ──
function Test-Pwsh7Installed {
    ...
    try {
        ...
        # Se não conseguir ler a versão, assume que o binário encontrado já é válido
        return $true
    }
}
...
# ── Helper de rede com retry (chamadas transitórias são o ponto mais frágil) ──
...
# Detecta arquitetura real do SO (mesmo se o processo atual for 32-bit)
...
# ── 2. Busca do release via API do GitHub ──
try {
...
# ── 3. Download direto via .NET Stream + instalação silenciosa ──
