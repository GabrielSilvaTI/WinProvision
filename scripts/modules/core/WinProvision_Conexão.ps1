...
.PARAMETER InternetTarget
    Host usado para checar conectividade antes de prosseguir (gate de rede).
.PARAMETER InternetMaxAttempts
    Número máximo de tentativas de checagem de conectividade.
.PARAMETER InternetIntervalSeconds
    Intervalo em segundos entre tentativas de checagem de conectividade.
...
param(
    ...
    [ValidateRange(1, 60)]
    [int]$InternetMaxAttempts = 10,

    [ValidateRange(1, 60)]
    [int]$InternetIntervalSeconds = 3,
...
function Wait-ForInternetConnection {
    <#
    .SYNOPSIS
        Gate de conectividade: aguarda a rede ficar disponível antes de prosseguir.
    .DESCRIPTION
        Faz até -MaxAttempts tentativas de ping contra -Target, aguardando
        -IntervalSeconds entre elas. Pensado para absorver os milissegundos de
        instabilidade de rede comuns na transição entre passes do Windows Setup
        (specialize -> oobeSystem -> primeiro logon), antes que os passos
        seguintes (download de módulos, possível re-fetch do próprio Bootstrap)
        dependam de rede.
    .NOTES
        ...
        HTTP/DNS funcionam normalmente, isso pode reportar "sem internet"
        incorretamente (falso negativo). Se o ambiente bloquear ICMP, troque para
        um teste baseado em TCP (ex.: porta 443).
    .OUTPUTS
        System.Boolean. $true se a conectividade foi confirmada, $false se
    ...
    for ($attempt = 1; $attempt -le $MaxAttempts; $attempt++) {
        Write-Step -Message "Verificando conectividade ($attempt/$MaxAttempts) contra $Target..." -Color Yellow
        if (Test-Connection -ComputerName $Target -Count 1 -Quiet -ErrorAction SilentlyContinue) {
            Write-Step -Message 'Conectividade confirmada.' -Color Green
            return $true
        }
...
function Start-Bootstrap {
    ...
    try {
        # 0. Gate de conectividade — absorve instabilidade de rede momentânea na
        #    transição entre passes do Windows Setup, antes de qualquer passo que
        #    dependa de rede (elevação via pipe, download de módulos, etc.).
        if (-not (Wait-ForInternetConnection -Target $InternetTarget -MaxAttempts $InternetMaxAttempts -IntervalSeconds $InternetIntervalSeconds)) {
            throw "Sem conectividade com a internet após $InternetMaxAttempts tentativa(s) (alvo: $InternetTarget)."
        }
