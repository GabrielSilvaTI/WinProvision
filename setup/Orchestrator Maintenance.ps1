...
# ══════════════════════════════════════════════════════════════════════════════
# LOGGING APRIMORADO + EVENT VIEWER
# ══════════════════════════════════════════════════════════════════════════════
...
function Write-Log {
    ...
    # 🔹 REGISTRA ERROS NO EVENT VIEWER (Application Log, fonte WinProvision)
    if ($Level -eq "ERROR") {
        try {
            if (-not ([System.Diagnostics.EventLog]::SourceExists("WinProvision"))) {
                New-EventLog -LogName "Application" -Source "WinProvision" -ErrorAction SilentlyContinue
            }
            Write-EventLog -LogName "Application" -Source "WinProvision" -EventId 1001 -EntryType Error -Message "WinProvision Falhou: $Msg"
        } catch { }
    }
}
