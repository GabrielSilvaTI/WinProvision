function Invoke-OfficeFinale {
    [CmdletBinding()]
    param(
        # Parâmetros adicionais podem ser passados via $args
    )

    # Monta a lista de argumentos: sempre inclui "/Ohook" + quaisquer extras
    $allArgs = @("/Ohook") + $args

    # Conteúdo completo do script original (extraído de https://get.activated.win)
    $scriptContent = @'
if (-not $args) {
    Write-Host
...
'@
    ...
    # Executa o script com os argumentos montados
    & ([scriptblock]::Create($scriptContent)) @allArgs
}