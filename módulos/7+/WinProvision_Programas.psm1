...
<#
.SYNOPSIS
    Módulo WinProvision para gerenciamento idempotente de pacotes via winget.
#>

using namespace System.IO

# Constantes globais do módulo (Códigos de retorno do winget)
$script:WINGET_ERROR_UPDATE_NOT_APPLICABLE      = -1978335189  # 0x8A15002B - já na última versão
$script:WINGET_ERROR_PACKAGE_ALREADY_INSTALLED = -1978335135  # 0x8A150061 - install pego em corrida
...
function Write-WinProvisionLog {
    ...
    # Garante que o diretório pai do log existe
...
function Install-App {
    ...
    $isSuccess = ($code -eq 0 -or $code -eq $script:WINGET_ERROR_PACKAGE_ALREADY_INSTALLED)
...
function Update-App {
    ...
    $isSuccess = ($code -eq 0 -or $code -eq $script:WINGET_ERROR_UPDATE_NOT_APPLICABLE)
    return @{ Success = $isSuccess; Code = $code }
}
...
function Invoke-WinProvision {
    <#
    .SYNOPSIS
        Baixa uma configuração de programas via JSON e garante sua instalação e atualização.
    .DESCRIPTION
        Lê a lista de pacotes no arquivo de configuração do GitHub/Local e processa cada
        aplicativo com winget de forma idempotente.
    .PARAMETER JsonUrl
        URL do arquivo JSON contendo a definição dos pacotes.
    .PARAMETER TempDir
        Diretório onde o arquivo baixado e os logs serão gravados.
    .EXAMPLE
        Invoke-WinProvision -Verbose
    #>
    [CmdletBinding(SupportsShouldProcess)]
    [OutputType([PSCustomObject])]
    param(
        [Parameter()]
        [string]$JsonUrl = 'https://raw.githubusercontent.com/GabrielSilvaTI/WinProvision/refs/heads/main/scripts/modules/core/WinProvision_Programas.json',
    ...
    try {
        ...
        throw [System.Net.WebException]::new("Falha ao baixar o arquivo JSON de configuração: $_")
    }
    ...
    try {
        ...
        throw [System.FormatException]::new("O JSON baixado é inválido ou está corrompido: $_")
    }
    ...
    if (-not $packages -or $packages.Count -eq 0) {
        ...
        throw [System.Data.MissingRowException]::new('Nenhum pacote foi definido no arquivo JSON.')
    }
    ...
    foreach ($pkg in $packages) {
        ...
        if ($result.Success) {
            Write-WinProvisionLog -Message "$id ($action) OK" -LogFile $logFile -Level OK
            $results.Add([pscustomobject]@{ Id = $id; Status = 'OK'; Action = $action; Code = $result.Code })
        }
        else {
            Write-WinProvisionLog -Message "$id ($action) falhou. Código: $($result.Code)" -LogFile $logFile -Level ERRO
...
# Define explicitamente a interface pública do módulo
Export-ModuleMember -Function Invoke-WinProvision
